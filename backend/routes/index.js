...
router.post("/add", async function (req, res, next) {
  ...
  if (todo_content) {
    try {
      ...
      var user = await db
        .prepare("SELECT * FROM users where address = (?)")
        .get(req.body.currentAcc);
      const todoList = await TodoList.deployed();
      await todoList.newTask.sendTransaction(todo_content, user.id, {
        from: req.body.currentAcc,
        gas: GAS_LIMIT,
      });
  ...
  error = "";
  res.redirect(
    "/?error=" + encodeURIComponent(error) + "&address=" + req.body.currentAcc
  );
  return;
});
...
router.post("/edit", async function (req, res, next) {
  ...
  try {
    ...
    console.log(user);
    const todoList = await TodoList.deployed();
    await todoList.editTask.sendTransaction(
      todo_id,
      task,
      user.id,
      Boolean(completed),
      { from: currentAcc, gas: GAS_LIMIT }
    );
  } catch (err) {
    console.log(err);
    return res.redirect(
      "/edit?address=" + req.body.currentAcc + "&todo_id=" + todo_id
    );
  }
